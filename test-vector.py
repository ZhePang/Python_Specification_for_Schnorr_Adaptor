...
def vector8():
    ...
    sig64 = schnorr_adapt(sig, t)
    return (t, sig, sig64, "TRUE", "Adaptor extraction and Adapting test: adaptor t is a large scalar")
...
# Converts the byte strings of a presig vector into hex strings
def presig_vector_to_hex(seckey, pubkey, aux_rand, msg, T, sig, result, comment):
    return (
        seckey.hex().upper() if seckey is not None else None,
        pubkey.hex().upper(),
...
# Converts the byte strings of a adapt-secadaptor vector into hex strings
def adapt_secadaptor_vector_to_hex(t, sig, sig64, result, comment):
    return (
...
presig_vectors_hex = list(map(lambda vector: presig_vector_to_hex(vector[0], vector[1], vector[2], vector[3], vector[4], vector[5], vector[6], vector[7]), presig_vectors))

adapt_secadaptor_vectors_hex = list(map(lambda vector: adapt_secadaptor_vector_to_hex(vector[0], vector[1], vector[2], vector[3], vector[4]), adapt_secadaptor_vectors))

def print_csv(vectors, is_presig):
    writer = csv.writer(sys.stdout)
    if is_presig:
        writer.writerow(("index", "secret key", "public key", "aux_rand", "message", "adaptor", "pre-signature", "result", "comment"))
    else:
        writer.writerow(("index", "secadaptor", "pre-signature", "BIP340 signature", "result", "comment"))
    for (i,v) in enumerate(vectors):
        writer.writerow((i,)+v)

if __name__ == "__main__":
    # Contains test vectors for `schnorr_presig_sign` and `schnorr_extract_adaptor` APIs
    out1 = "presig_vectors.csv"
    # Contains test vectors for `schnorr_adapt` and `schnorr_extract_secadaptor` APIs
    out2 = "adapt_secadaptor_vectors.csv"

    with open(out1, "w") as f:
        sys.stdout = f
        print_csv(presig_vectors_hex, True)

    with open(out2, "w") as f:
        sys.stdout = f
        print_csv(adapt_secadaptor_vectors_hex, False)